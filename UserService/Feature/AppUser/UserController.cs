...
public class UsersController(UserService userService) : ControllerBase
{
    ...
    /// </summary>
    /// <param name="request">The <see cref="RegisterUserRequest"/>.</param>
    /// <returns>A <see cref="Task"/> with the <see cref="IActionResult"/>.</returns>
    [AllowAnonymous]
    [HttpPost("register")]
    public async Task<IActionResult> Register([FromBody] RegisterUserRequest request)
    {
        // Register
        var response = await userService.CreateUserAsync(request.Username, request.Email, request.Password, request.FirstName, request.LastName);
